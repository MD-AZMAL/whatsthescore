...
def getscore():
    ...
    while True:
        ...
        match = soup.find_all('a',{'class':'text-hvr-underline'})[0].text[:-1]

        if scoreDiv[-1] != ')':
            scoreDiv = scoreDiv + 'yet to bat'
        
        try:
            additionalMessage = soup.find_all('div',{'class': 'cb-text-live'})[0].text
        except:
            try:
                additionalMessage =  soup.find_all('div',{'class': 'cb-text-complete'})[0].text
            except:
                additionalMessage = ''
